...
class Documents extends DbTable
{
    ...
    // Fields
    public DbField $id;
    public DbField $procurement_id;
    public DbField $file_name;
    public DbField $file_path;
    public DbField $uploaded_at;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "documents"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // procurement_id
        $this->procurement_id = new DbField(
            $this, // Table
            'x_procurement_id', // Variable name
            'procurement_id', // Name
            '`procurement_id`', // Expression
            '`procurement_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`procurement_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->file_name->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->uploaded_at->Nullable = false; // NOT NULL field
        $this->uploaded_at->Required = true; // Required field
        $this->uploaded_at->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));
        $this->uploaded_at->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->procurement_id->DbValue = $row['procurement_id'];
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("documentslist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "documentsview" => $this->language->phrase("View"),
            "documentsedit" => $this->language->phrase("Edit"),
            "documentsadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->procurement_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->procurement_id);
