...
class DocumentsEdit extends Documents
{
    ...
    // Page object name
    public string $PageObjName = "DocumentsEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "documentsedit";
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("documentslist"); // Return to list page
                        return;
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'procurement_id' before field var 'x_procurement_id'
        $val = $this->hasFormValue("procurement_id") ? $this->getFormValue("procurement_id") : $this->getFormValue("x_procurement_id");
        if (!$this->procurement_id->IsDetailKey) {
            if (IsApi() && $val === null) {
        ...
        // Check field name 'uploaded_at' before field var 'x_uploaded_at'
        $val = $this->hasFormValue("uploaded_at") ? $this->getFormValue("uploaded_at") : $this->getFormValue("x_uploaded_at");
    ...
    public function renderRow(): void
    {
        ...
        // procurement_id
        $this->procurement_id->RowCssClass = "row";
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->procurement_id->Visible && $this->procurement_id->Required) {
            ...
            if ($this->file_path->Visible && $this->file_path->Required) {
                if (!$this->file_path->IsDetailKey && IsEmpty($this->file_path->FormValue)) {
            ...
            if ($this->uploaded_at->Visible && $this->uploaded_at->Required) {
                if (!$this->uploaded_at->IsDetailKey && IsEmpty($this->uploaded_at->FormValue)) {
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // uploaded_at
        $this->uploaded_at->setDbValueDef($newRow, UnFormatDateTime($this->uploaded_at->CurrentValue, $this->uploaded_at->formatPattern()), $this->uploaded_at->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['procurement_id'])) { // procurement_id
            $this->procurement_id->CurrentValue = $row['procurement_id'];
        }
